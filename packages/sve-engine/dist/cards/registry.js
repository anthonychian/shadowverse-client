...
const handAuthored = {};
function loadHandAuthoredJson(dir) {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (!entry.name.endsWith(".json"))
            continue;
        if (entry.name === "parsed-stubs.json")
            continue;
...
function overlayRichness(overlay) {
...
for (const [cardNo, overlay] of Object.entries(handAuthored)) {
    ...
    const merged = (0, reprints_1.mergeSharedHandOverlays)(prev, (0, reprints_1.pickSharedHandOverlay)(overlay));
...
for (const [cardNo, overlay] of Object.entries(handAuthored)) {
    ...
    const isEvolvedPrinting = overlay.printingType === "evolved" ||
    ...
    registerCardDef(cardNo, (0, reprints_1.mergePrintingWithGameplay)(printingStub, printingStub, handOverlayForPrinting(cardNo, printingStub)));
}
