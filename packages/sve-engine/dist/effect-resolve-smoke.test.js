...
function walkEffects(effect, out) {
    if (!effect)
        return;
    out.push(effect);
    if (effect.op === "sequence")
        effect.steps?.forEach((s) => walkEffects(s, out));
...
function effectTreeIsValid(effect) {
    if (!effect)
        return null;
    if (effect.op === "noop") {
    ...
    if (!KNOWN_OPS.has(effect.op))
        return `unknown op ${effect.op}`;
    if (effect.op === "sequence") {
    ...
    if (effect.op === "if") {
        ...
        if (t)
            return t;
        if (effect.else)
            return effectTreeIsValid(effect.else);
    }
    if (effect.op === "optionalCost") {
...
(0, vitest_1.describe)("effect resolve smoke", () => {
    const DECK_CARDS = [
    ...
    (0, vitest_1.it)("deck cards have known ops and no noop stubs", () => {
        const deckSet = new Set(DECK_CARDS);
        const defs = (0, registry_1.getAllCardDefs)().filter((d) => deckSet.has(d.cardNo) && d.abilities?.length && !VANGUARD_RE.test(d.cardText || ""));
        const unknownOps = [];
        for (const def of defs) {
            for (const ability of def.abilities ?? []) {
                ...
                if (err)
                    unknownOps.push(`${def.cardNo} ${err}`);
            }
        }
        (0, vitest_1.expect)(unknownOps, unknownOps.slice(0, 30).join("\n")).toEqual([]);
    });
    (0, vitest_1.it)("non-Vanguard cards have known ops structurally resolvable", () => {
        const defs = (0, registry_1.getAllCardDefs)().filter((d) => d.abilities?.length && !VANGUARD_RE.test(d.cardText || ""));
        const unknownOps = [];
        for (const def of defs) {
            for (const ability of def.abilities ?? []) {
                const err = effectTreeIsValid(ability.effect);
