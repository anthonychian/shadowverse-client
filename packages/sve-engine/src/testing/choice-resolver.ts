...
function findInstanceIdByCardNo(
  state: GameState,
  player: PlayerId,
  cardNo: string,
  zone?: string,
  ...
  const zones = zone
    ? { [zone]: state.players[player].zones[zone as keyof typeof state.players[0]["zones"]] }
    : state.players[player].zones;
  for (const [zoneName, list] of Object.entries(zones)) {
    if (!Array.isArray(list)) continue;
    const card = (list as { instanceId: string; cardNo: string }[]).find((c) => c.cardNo === cardNo);
    if (card) return card.instanceId;
  }
  return null;
}

export function resolveChoicesWithHints(
  state: GameState,
  player: PlayerId,
  hints: ChoiceHints = {},
  maxSteps = 40,
  ...
  for (let step = 0; step < maxSteps; step++) {
    if (!current.pendingChoices && current.pendingTriggers.length > 0) {
      current = runConfirmationTiming(current);
    }
    ...
    if (choice.type === "selectTarget") {
      let targetId: string | "leader" = "leader";
      if (hints.targetCardNo === "leader") {
        targetId = "leader";
      } else if (hints.targetCardNo) {
        const id =
          findInstanceIdByCardNo(current, player, hints.targetCardNo, "field") ??
          findInstanceIdByCardNo(current, 1 - player as PlayerId, hints.targetCardNo, "field");
        if (id) targetId = id;
      } else {
    ...
    if (choice.type === "selectEvolveDeckCard") {
      ...
      if (!resolved.ok) break;
      current = runConfirmationTiming(resolved.state);
      continue;
    }
    ...
    if (choice.type === "selectZoneCards") {
      ...
      if (picked.length < (choice.minCount ?? pickCount) || picked.length > pickCount) break;
      const resolved = applyAction(current, player, {
        type: "CHOICE_RESPONSE",
...
export function hintsFromAction(action: ScenarioAction): ChoiceHints {
